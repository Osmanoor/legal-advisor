...
def load_articles_data(file_path: str) -> Dict[int, Dict]:
    """
    Load article data from JSON file and index by article number
    
    Args:
        file_path: Path to JSON file with articles data
        
    Returns:
        Dictionary mapping article numbers to article data
    """
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        
        # Create lookup dictionary by article number
        articles_by_number = {}
        for article in data:
            if article.get('type') == 'اللائحة':  # Only include regulation type
                article_num = article.get('number')
                if article_num:
                    articles_by_number[article_num] = article
        
        return articles_by_number
...
class ProcurementResult:
    ...
    referenced_articles: List[int]
    referenced_articles_data: List[Dict]  # Full article data
...
class ProcurementCalculator:
    ...
    def calculate_performance_guarantee(self, procurement_type: ProcurementType, budget: float) -> Union[int, str]:
        ...
        if procurement_type in [ProcurementType.DIRECT_PURCHASE, 
            ...
            return 5 if budget >= 500000 else 0
    ...
    def calculate(self, input_data: ProcurementInput, articles_data: Dict[int, Dict] = None) -> ProcurementResult:
        """Calculate complete procurement information with article references"""
        ...
        # Get detailed article data if available
        referenced_articles_data = []
        if articles_data:
            for article_num in referenced_articles:
                if article_num in articles_data:
                    referenced_articles_data.append(articles_data[article_num])
        ...
        result = ProcurementResult(
            ...
            referenced_articles_data=referenced_articles_data,
            stages=[],
            total_duration=0
        )
...
class ProcurementSystem:
    ...
    def __init__(self, config_file: str = None, articles_file: str = None):
        # Load configuration from file if provided
        self.config = ProcurementConfig()
        self.calculator = ProcurementCalculator(self.config)
        
        # Load articles data if provided
        self.articles_data = {}
        if articles_file:
            self.articles_data = load_articles_data(articles_file)
    ...
    def process_input(self, work_type: str, budget: float, start_date: str,
        ...
        # Calculate result with articles data
        result = self.calculator.calculate(input_data, self.articles_data)
    ...
    def _result_to_dict(self, result: ProcurementResult) -> Dict:
        ...
        # Convert article data to serializable format
        articles_data = []
        for article in result.referenced_articles_data:
        ...
        return {
            ...
            "referenced_articles_data": articles_data,
            "stages": stages_dict,
