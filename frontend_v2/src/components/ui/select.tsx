...
>(({ className, children, ...props }, ref) => (
  ...
  >
    {/* 
      MODIFICATION: 
      - The wrapper div ensures the text area has flexible width.
      - `truncate` adds the ellipsis for overflow.
      - `text-right` is the crucial fix. It aligns the text correctly for RTL forms, 
        allowing the ellipsis to appear on the left as expected.
    */}
    <div className="flex-1 truncate text-right">{children}</div>
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-default select-none items-center rounded-sm py-2 pl-8 pr-2 text-sm outline-none focus:bg-gray-100 focus:text-gray-900 data-[disabled]:pointer-events-none data-[disabled]:opacity-50 whitespace-normal",
      className
    )}
    {...props}
  >
